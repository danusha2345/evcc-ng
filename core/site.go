...
func (site *Site) update(lp updater) {
	...
	// Dim/curtail PV. HEMS supplies dim and curtail tristate (*bool, nil = not
	// managed); we additionally curtail on feed-in price below threshold
	// (evcc-io/evcc#21747). Both curtail inputs are tristate: a nil result
	// leaves the inverter's registers untouched so an external/static limit
	// — e.g. Huawei's 70% cap via FusionSolar on the same registers — is
	// preserved (#30068, #30116/#30712 tristate API). Feed-in curtail must run
	// even without a HEMS, so the block is gated on either being managed.
	...
	if site.hems != nil || feedinCurtail != nil {
		...
		if feedinCurtail != nil {
			v := *feedinCurtail || (curtail != nil && *curtail)
			curtail = &v
		}
