...
func (site *Site) update(lp updater) {
	...
	// Dim/curtail PV. HEMS supplies dim (*bool) and a curtail percent (*int, nil
	// = not managed; upstream #31390 distributes limits by percent). We
	// additionally curtail on feed-in price below threshold (evcc-io/evcc#21747).
	// Both curtail inputs are tristate: a nil result leaves the inverter's
	// registers untouched so an external/static limit — e.g. Huawei's 70% cap via
	// FusionSolar on the same registers — is preserved (#30068, #30116/#31390).
	// Feed-in curtail must run even without a HEMS, so the block is gated on
	// either being managed.
	feedinCurtail := site.shouldFeedInCurtail()
	if site.hems != nil || feedinCurtail != nil {
		curtail := hemsCurtailed(site.hems)
		if feedinCurtail != nil {
			// map our feed-in decision onto the percent API: curtail = fully
			// stop export (0%), don't curtail = full output (100%). The more
			// aggressive of feed-in and HEMS (lower percent) wins.
			p := 100
			if *feedinCurtail {
				p = 0
			}
			if curtail == nil || p < *curtail {
				curtail = &p
			}
		}
